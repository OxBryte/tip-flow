/**
 *Submitted for verification at basescan.org on 2025-05-30
*/
...
// File: Tipn/IAction.sol


pragma solidity ^0.8.0;

interface IAction {
    function onTip(address from, address to, address cast, uint quantity, bytes calldata data) external;
}
// File: Tipn/TipnV2.sol
...
contract TipnV2 is Ownable {
    ...
    function getTipperAt(uint index) external view returns (address) {
        return _tippers.at(index);
    }
    function getNumTippers() external view returns (uint) {
    ...
    function getCastTipperAt(address cast, uint index) external view returns (address) {
        return _castTippers[cast].at(index);
    }
    function getNumCastTippers(address cast) external view returns (uint) {
        return _castTippers[cast].length();
    }
