...
async function testNotificationSystem() {
  ...
  try {
    ...
    if (usersData.success) {
      console.log(
        `✅ Found ${usersData.totalUsers} users with notification tokens:`
      );
    ...
    const statusResponse = await fetch(
      `${BACKEND_URL}/api/notification-status/${testUserAddress}`
    );
    ...
    const testNotificationResponse = await fetch(
      `${BACKEND_URL}/api/test-notification`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          message: "This is a test notification from Tip Flow!",
