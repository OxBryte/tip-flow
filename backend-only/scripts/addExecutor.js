// Script to add backend wallet as executor to Tip FlowBatch contract
...
async function addExecutor() {
  try {
    ...
    if (owner.toLowerCase() !== wallet.address.toLowerCase()) {
      console.log(
        `❌ Backend wallet is not the contract owner. Only the owner can add executors.`
      );
      console.log(
        `💡 You need to call addExecutor(${wallet.address}) from the owner wallet.`
      );
      return;
    }
