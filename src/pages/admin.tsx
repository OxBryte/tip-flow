...
export default function Admin() {
  ...
  const fetchAdminData = async () => {
    try {
      ...
      if (!statsResponse.ok) {
        throw new Error(
          `Stats request failed with status ${statsResponse.status}`
        );
      }
      const statsData = await statsResponse.json();
      const payload =
        (statsData.stats as AdminStats | undefined) ??
        (statsData.data as AdminStats | undefined);
      ...
      const tipsResponse = await fetch(
        `${BACKEND_URL}/api/admin/recent-tips?limit=20`
      );
      if (!tipsResponse.ok) {
        throw new Error(
          `Recent tips request failed with status ${tipsResponse.status}`
        );
      }
    ...
    <div className="min-h-screen bg-yellow-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Live tipping statistics for Tip Flow</p>
          {lastUpdated && (
            ...
            >
              <div className="px-6 py-4 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Rolling totals for key timeframes
                </p>
              </div>
                ...
                {timeframeCards.map((card) => (
                  <div key={card.label} className="p-6 text-center">
                    ...
                    <p className="text-2xl font-semibold text-gray-900 mt-2">
                      {card.getTips(stats)} tips
                    </p>
                    <p className="text-sm text-gray-600 mt-1">
                      {card.getUsdc(stats)} USDC
                    </p>
                  </div>
                ))}
              </div>
